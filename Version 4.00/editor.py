            ...
            while len(linea)>1:
                ...
                if linea[0].upper()==LETRA_MARCAS:
                    ...
                    while linea[0].upper()==LETRA_NOTAS:
                        ...
                        for c in linea[1]:
                            t.agregar_nota(c==NOTA_ACTIVADA)
...
#--------------------------REPRESENTAR Y GUARDAR-------------------------#
    def representar_cancion(self):
        ...
        print(self.t_act)
        print(self.iterador.pila.lista)
        print(self.tiempos)
        ...
        print()
        for x in range(len(self.t_act.obtener_notas())):
            print(x, end='  ')
            for t in self.tiempos:
                lista_notas=t.obtener_notas()
                nota=lista_notas[x]
...
#----------------------------REPRODUCCION----------------------------#
    def obtener_sonidos(self,marca_tiempo):
        ...
        Post:
            ...
            de la clase SoundFactory."""
        lista_notas=marca_tiempo.obtener_notas()
        sonidos=[]
...
#--------------------MANEJO DE MARCAS DE TIEMPO-------------------------#
    ...
    def anadir_marca_prev(self,duracion):
        ...
        if self.pos_t==0:
            ...
            return
        self.retroceder()
        self.anadir_marca_next(duracion)
        self.avanzar(2)
        self.pos+=1
...
#-------------------------MANEJO DE NOTAS----------------------------#
    ...
    def cambiar_estado(self,pos,estado):
        '''Toma como parametro una posicion y activa la nota en dicha posicion.
        Pre: pos debe ser un numero int'''
        dato=self.t_act.index_notas(pos)
        if dato==estado:
            raise ValueError('No se puede realizar la operacion para el track elegido.')
        self.t_act.quitar_nota(pos)
        self.t_act.agregar_nota(estado,pos)
